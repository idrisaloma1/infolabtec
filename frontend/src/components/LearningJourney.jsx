          ...
          {STEPS.map((step, i) => (
            <div key={step.title} className="flex-1 flex items-center gap-4 md:flex-col md:text-center">
              <div className="flex flex-col items-center md:contents">
                <div className="h-12 w-12 shrink-0 rounded-full bg-gradient-to-br from-electric-500 to-cyan-400 flex items-center justify-center font-display font-bold text-navy-950">
                  {i + 1}
                </div>
              </div>
              <div>
                <h3 className="font-display font-semibold text-base md:mt-4">{step.title}</h3>
                <p className="mt-1 text-xs md:text-sm text-white/50 max-w-[16rem]">{step.desc}</p>
              </div>
              {i < STEPS.length - 1 && (
